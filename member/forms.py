...
from member.models import Vcode
from utility.d_discord import send_msg
from utility.d_utils import (
    check_for_duplicate_signup,
    check_if_student_id_exists,
    validation,
)
...
class SocialSignupForm(SignupForm):
    ...
    student_id = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                ...
                "placeholder": f"{timezone.now().year}112000",
                "required": "",
            }
        ),
    )
    ...
    def pre_save(self, request):
        if request.method == "POST":
            ...
            if check_for_duplicate_signup(raw_data):
                send_msg(request, "duplicate sign up attempted")
                return None
            elif check_if_student_id_exists(raw_data) == False:
                send_msg(request, "sign up requested with invalid student id")
                return None
